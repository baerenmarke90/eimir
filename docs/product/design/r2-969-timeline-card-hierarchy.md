...
3. **Bottom bar clearance (with #970).** The floating bottom bar hides on deliberate downward scrolling and is revealed after a small upward scroll. Pinning is independent of it: the pinned heading keeps its position while the bar hides or reveals, and the two never overlap. A keyboard focus move that scrolls only a few pixels can leave the bar revealed, which let it cover most of a focused card near the bottom. Timeline card links therefore get `scroll-margin-bottom` equal to the shell's `--shell-floating-nav-footprint` wherever the bottom shell exists (`@container product-shell (max-width: 52.4375rem)`). Without it, the integrated spec fails (focused card bottom 844 px against a revealed bar top of 772 px); with it, the card clears the bar.
...
- **Web browser:** the new `r2-timeline-card-hierarchy.spec.ts` covers the #970 integration (bar hides on scroll-down and reveals on scroll-up while the heading stays pinned without overlap, content at the end of the Timeline and a keyboard-focused card clear of a revealed bar, dark, Expanded without a bottom bar, 320 px and 200% text without collisions or overflow) as well as no pills, no stripe, the kind nodes, named icons, first child at the card padding, footer and title containment at 390/320 px and 200% text, pinned and handed-over headings, the zoom fallback, focus clearance, Planen's bar unchanged, and axe at WCAG 2.2 AA in light and dark. The existing Timeline, Story years, Discover, featured metadata, i18n names, R2 evidence and F2 task-boundary specs were re-run one file at a time with `--workers=1`. `story-timeline-attachment-meta.spec.ts` now asserts the wrapping row instead of the old stacked column.
...
- Integrated with #970 (captured on the rebased branch): `09`/`10` refreshed because scrolling now hides the bottom bar, plus `13` bar hidden with heading pinned, `14` bar revealed with heading unchanged, `15` end of Timeline with revealed bar, `16` keyboard focus clear of the bar, `17` dark, `18` Expanded 1280 px (no bottom bar), `19` 320 px, `20` 320 px at 200% text. The other Web evidence was not affected by #970: it was captured at the top of the page (full-page) or as centred card crops, and was left unchanged.
...
- At 320 px with 200% root text, the shell's top bar (189 px) and floating bottom bar take most of a 640 px viewport, and the bottom bar's labels truncate (`20-integrated-320-200pct-text-bar-revealed.png`). Pinning correctly falls back, so a month heading scrolls under the top bar as ordinary content there. The shell bars are owned by the app shell and #970, not by this issue.
