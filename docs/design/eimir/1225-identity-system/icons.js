/* Illustration of the owner-selected 24 px grammar; runtime names and paths are checked by verify.py. */
const ICON_PATHS = {
  ...
  kalender: '<rect x="3.5" y="5.5" width="17" height="15" rx="2.7"/><path d="M3.5 10h17M8 3.5v4M16 3.5v4"/><path class="accent" d="M8 3.5v4M16 3.5v4"/>',
  ...
  geteilt: '<path class="icon-fill-accent" d="M12 20.5S3.5 15.3 3.5 9.5a4.6 4.6 0 0 1 8.5-2.4 4.6 4.6 0 0 1 8.5 2.4c0 5.8-8.5 11-8.5 11Z"/><path d="M3.9 13.2c4.5 6.5 10.2 8.5 16.2 0"/>',
  ...
  meilensteine: '<path class="accent" d="m13 2.5 6 6-6 6-6-6 6-6Z"/><path d="m8.5 10 6 6-6 6-6-6 6-6Z"/>',
...
const FILLED = {
  ...
  momente: '<path d="M12 20.5S3.5 15.3 3.5 9.5a4.6 4.6 0 0 1 8.5-2.4 4.6 4.6 0 0 1 8.5 2.4c0 5.8-8.5 11-8.5 11Z" fill="url(#__HEART__)" stroke="none"/>',
  ...
  nurfuer: '<rect x="4.4" y="10" width="15.2" height="11" rx="2.7" fill="url(#__LOCK__)" stroke="none"/><path d="M7.5 10V7a4.5 4.5 0 0 1 9 0v3" stroke="#D577B9"/><path d="M12 14v4m-2-2h4" stroke="#fff"/>',
};
...
const DUOTONE = {
  ...
  nurfuer: '<rect x="4.4" y="10" width="15.2" height="11" rx="2.7" fill="url(#__LOCK__)" stroke="none"/><path d="M7.5 10V7a4.5 4.5 0 0 1 9 0v3" stroke="#E39DC7"/><path d="M12 14v4m-2-2h4" stroke="#fff"/>',
};

let iconSequence = 0;
function icon(name, variant = 'outline', size = 24) {
  const source = variant === 'pressed' && name === 'momente'
    ? `${FILLED.momente}<path d="M7.4 17.1c1.5 1.3 3.1 2.5 4.6 3.4 2.1-1.3 4.5-3.2 6.1-5.2" fill="none" stroke="#759FE5" stroke-width="1.8"/>`
    : variant === 'filled' ? (FILLED[name] || ICON_PATHS[name]) : variant === 'duotone' ? (DUOTONE[name] || ICON_PATHS[name]) : ICON_PATHS[name];
  if (!source) throw new Error(`Unknown icon: ${name}`);
  const id = `owner-heart-${++iconSequence}`;
  const stops = variant === 'duotone'
    ? '<stop stop-color="#8FB3F3"/><stop offset="1" stop-color="#F5A3BC"/>'
    : '<stop stop-color="#8A83DE"/><stop offset=".58" stop-color="#D185D4"/><stop offset="1" stop-color="#F58AAE"/>';
  const defs = source.includes('__HEART__')
    ? `<defs><linearGradient id="${id}" x1="0%" y1="100%" x2="100%" y2="0%">${stops}</linearGradient></defs>`
    : source.includes('__LOCK__')
      ? `<defs><linearGradient id="${id}" x1="0%" y1="0%" x2="0%" y2="100%"><stop stop-color="${variant === 'filled' ? '#E98AC5' : '#8FB3F3'}"/><stop offset="1" stop-color="${variant === 'filled' ? '#74A9E7' : '#F5A3BC'}"/></linearGradient></defs>` : '';
  return `<svg class="eicon ${variant}" viewBox="0 0 24 24" width="${size}" height="${size}" fill="none" stroke="currentColor" stroke-width="1.65" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${defs}${source.replaceAll('__HEART__', id).replaceAll('__LOCK__', id)}</svg>`;
}
