...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            "POST",
            "/api/v1/auth/recent-authentication/server-admin/passkeys/start",
            None,
        ),
        (
            "POST",
        ...
        (
            "POST",
            "/api/v1/auth/recent-authentication/server-admin/oidc/test-oidc/start",
            None,
        ),
        (
            "POST",
            "/api/v1/auth/recent-authentication/server-admin/oidc/test-oidc/callback",
            {"code": "not-used", "state": "not-used"},
        ),
    ),
)
def test_server_admin_recent_auth_grant_endpoints_reject_normal_users(
    client,
    session: Session,
    server_admin_allowlist,
    method: str,
    ...
    normal = make_account(session, "Normal Grant Caller")
    normal_token = sign_in(session, normal)
...
def test_password_step_up_issues_server_admin_purpose_and_authorizes_action(
    ...
    assert (
        stepped_up.json()["method"] == recent_auth.RecentAuthenticationMethod.LOCAL_PASSWORD.value
    )
...
def test_recent_auth_purposes_are_strictly_isolated(
    ...
    assert (
        rejected_admin_action.json()["code"] == recent_auth.RecentAuthenticationErrorCode.REQUIRED
    )
    ...
    assert (
        rejected_account_action.json()["code"] == recent_auth.RecentAuthenticationErrorCode.REQUIRED
    )
...
def test_expired_server_admin_grant_fails_closed(
    ...
    grant.achieved_at = expired_at - recent_auth.RECENT_AUTH_LIFETIME
