...
test.describe('Browser Session Reload and Deep Route Restoration', () => {
  ...
  test('does not let a late refresh from a signed-out account replace the new session', async ({
    page,
  }) => {
    const SECOND_ACCOUNT_ID = '00000000-0000-0000-0000-000000000003';
    let signInCount = 0;
    ...
    await page.route('**/api/v1/**', async (route) => {
      ...
      if (method === 'POST' && pathname === '/api/v1/auth/sign-in') {
        signInCount += 1;
        const secondSignIn = signInCount === 2;
        await fulfillJson({
          account: {
            displayName: secondSignIn ? 'Bea' : 'Anna',
            id: secondSignIn ? SECOND_ACCOUNT_ID : ACCOUNT_ID,
          },
          tokens: {
            accessExpiresAt: new Date(
              Date.now() + (secondSignIn ? 60 * 60_000 : 30_000),
            ).toISOString(),
            accessToken: secondSignIn ? 'access-b' : 'access-a',
            refreshExpiresAt: new Date(
              Date.now() + 7 * 24 * 60 * 60_000,
            ).toISOString(),
            refreshToken: secondSignIn ? 'refresh-b' : 'refresh-a',
          },
        });
        return;
      }
      ...
      if (method === 'POST' && pathname === '/api/v1/auth/refresh') {
        ...
        if (body.refreshToken === 'refresh-a') {
          ...
          await fulfillJson({
            accessExpiresAt: new Date(Date.now() + 60 * 60_000).toISOString(),
        ...
        if (body.refreshToken === 'refresh-b') {
          await fulfillJson({
            accessExpiresAt: new Date(Date.now() + 60 * 60_000).toISOString(),
            accessToken: 'refreshed-access-b',
            refreshExpiresAt: new Date(
              Date.now() + 7 * 24 * 60 * 60_000,
            ).toISOString(),
      ...
      if (method === 'GET' && pathname === '/api/v1/auth/me') {
        const secondAccount = authorization === 'Bearer access-b';
        await fulfillJson({
          id: secondAccount ? SECOND_ACCOUNT_ID : ACCOUNT_ID,
          displayName: secondAccount ? 'Bea' : 'Anna',
        });
        return;
      }
      ...
      if (
        ...
        await fulfillJson(
          {
            canManageSpaceConfiguration: false,
      ...
      if (
        ...
        const currentAccountId =
          authorization === 'Bearer access-b' ? SECOND_ACCOUNT_ID : ACCOUNT_ID;
        await fulfillJson({
          accountId: isPartner ? PARTNER_ID : currentAccountId,
          createdAt: TEST_NOW,
    ...
    await signIn(page, 'bea@example.org');
    await expect(
      page.getByRole('heading', { name: m5s5.today.headerTitle, level: 1 }),
    ).toBeVisible();

    const lateRefreshResponse = page.waitForResponse(
      (response) =>
    ...
    expect(storedSession).toMatchObject({
      account: { id: SECOND_ACCOUNT_ID, displayName: 'Bea' },
      tokens: {
