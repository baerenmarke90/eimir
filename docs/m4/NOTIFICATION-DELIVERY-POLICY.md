...
**Current baseline:** `main` at `f381262c`, 23 September 2026. This is
the first runtime slice of #515, not the complete notification-policy feature.
...
The only push preview remains `notification.generic`. The provider's
notification reference contains only the notification ID and kind, never
recipient-controlled plaintext, actor name, target title,
unrevealed-surprise hint or private content. The
...
**Not delivered by this slice:** #515's digest aggregation, rate/noise policy,
...
passes a stale queued delivery to the provider. No UI change or screenshot
is claimed in this backend-only slice.
