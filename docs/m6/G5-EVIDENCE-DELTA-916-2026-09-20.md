...
| 07 | ServerAdmin Access Gate | `/server-admin` (non-admin) | Gated shell rendered (`.server-admin-gate-shell`); status announced via `div.ui-state-permission[role="status"]`; accessible recovery link to `/today` keyboard reachable and operable. | 0 critical / 0 serious | `PASS` |
