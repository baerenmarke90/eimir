...
- Performance: bounded origin metadata, existing cached pages, no second content cache, release media URLs on actual task end. Native confirmed-save refresh targets the previous loaded count plus one page, stopping after a complete page reaches that target or the feed ends; ordinary detail return does not refetch that range. This adds no managed-service cost category or new quota. Do not pin all history indefinitely.
...
Native evidence identified unreadable retained-field text and insufficient Dark foreground contrast in the proof's detail date, Delete action and selected navigation. These consumers reuse existing readable text/accent roles while preserving disabled interaction, layout, action meaning and the palette. This bounded correction is part of result/return accessibility; broader accent propagation remains with the later reference/cleanup slices.
...
- Broader browser regression: eight existing expectations assumed the previous inline filter, floating navigation during capture, custom focus loop or post-save Story route. They now assert the F2 task contract. The complete hosted Chromium/axe suite passed **287/287** in 15.5 minutes on `527247ab8e303626a0788fa8cd1ed150b4726103`, with no failures, skips or flaky retries.
- Native behavioral baseline `fe55f230d558816d7e181c650d1b7035c6bd246c`: 596 unit tests across 89 suites, no failures/errors and one existing G2 skip. This includes 20 Memory-task regressions and five production-route journeys. Lint passed with zero errors and 54 baseline warnings; the debug APK and release-manifest/asset exclusions passed. The subsequent `0616ed02e2acf2ded76292ced51e1c83a420fb41` delta changes only the described foreground roles and the capture helper's localized modal lookup. Local dependency verification uses the documented lenient mode for the existing macOS AAPT2 metadata gap; CI retains strict verification.
- Final native runtime `0616ed02e2acf2ded76292ced51e1c83a420fb41`: the five production-route journeys and debug build passed again. The exact APK passed all 16 emulator behavior scenarios, with 34 unchanged captures across 320/360/390/430/1280 dp, Light/Dark, IME, rotation, background return, failure states, 200% text and reduced animation. All 36 measured controls meet 48 dp. Separate supplements prove system Photo Picker cancellation/selection, decoded saved photos in both themes, reconciliation after a partial binding failure, reachable scrolled actions at 200%, and a fully visible older Memory at identical bounds after Back without scrolling. These are deterministic transport proofs, not live-backend persistence claims.
- Independent review found and corrected an avatar callback generation mismatch and an unsafe Timeline refresh merge. Post-save refresh now atomically replaces an authoritative contiguous prefix through the existing scoped cursor API; transient failure retains the old range, denial clears it, and stale session/scope results cannot publish. Ordinary detail return retains its existing loaded range without this refresh.
- Product-design gate tests (26), engineering-language tests (22) and the scoped English-language audit passed. Native device evidence, including TalkBack service-state verification and its recorded automation limitation, is complete; see the [evidence README](evidence/f2/README.md#android). Required CI is the remaining gate before merge readiness.
